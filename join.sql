...
FROM books AS b /* Escojer una tabla de pivote */
INNER JOIN authors AS a /* Se va a relacionar con */
  ON a.author_id = b.author_id
WHERE a.author_id BETWEEN 1 AND 5;

SELECT c.name AS cliente, b.title AS libro, a.name AS autor, t.type AS estatus
...
En el caso de hacer JOINS entre tablas cuyo nombre siga el convention over configuration 
explicado por el profe, se puede usar USING(`nombre_campo`) en lugar del ON, es decir:

esto:

FROM table_1 AS t1 JOIN table_2 AS t2 
USING(`campo_en_comun_id`)

en lugar de:

FROM table_1 AS t1 
  JOIN table_2 AS t2 
  ON t1.`campo_en_comun_id` = t2.`campo_en_comun_id`
*/